...
// Returns true if addition succeeded.
    public boolean add(K key, V value){
        if(isFull() || contains(key))
            return false;
//        HashBruh temp = new HashBruh<K,V>(key,value);
//        HashBruh returnValue = hash_arr[getIndex(key)].find(temp);
//        if(returnValue !=null) return false;
//        hash_arr[getIndex(key)].addLast(temp);
    ...
    public boolean delete(K key){
        ...
        for(HashBruh<K,V> indx : hash_arr[idx]) {
            if(indx.key.equals(key)) {
                ...
                hsize--; //decreases the size of the array
                return true;
            }
        }
        return false;
